...
map_curl <- function(
  ...
  message(tr_(
    "Downloading %s in %i files...",
    format_bytes(sum(sizes)),
    length(files)
  ))
  ...
  make_handle <- function(i) {
    ...
    options <- utils::modifyList(
      list(low_speed_limit = low_speed_limit, low_speed_time = low_speed_time),
      options
    )
  ...
  map_pool <- curl::new_pool(
    total_con = total_connections,
