...
year_range_overlaps <- function(titles, year_range) {
  ...
  dates <- utils::strcapture(
    date_range_regex,
    titles,
    proto = list(file_date_start = integer(), file_date_end = integer())
  )
  ...
  file_inside_window <- (year_range[1] <= file_date_start) &
    (file_date_end <= year_range[2])
  # Is the window specified by the user within the file?
  window_inside_file <- (file_date_start <= year_range[1]) &
    (file_date_end >= year_range[2])
  # Does the window intersect the start of the file?
  left <- (year_range[1] <= file_date_start) &
    (file_date_start <= year_range[2])
  ...
  window_in_file <- file_touches_window |
    file_inside_window |
    window_inside_file
...
cmip6_file_template <- c(
  "%(variable_id)s_%(table_id)s_%(source_id)s_%(experiment_id)s_%(member_id)s_%(grid_label)s_%(time_start)s-%(time_end)s.nc",
